...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (globals) {
	  globals.Tracker = _tracker2['default'];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _cookiesJs = __webpack_require__(6);
	 ...
	 * @param accepts
	 */
	var json = function json(url, verb, data, cb, accepts) {
	  ...
	  _superagent2['default'][verb](url).set('Accept', accepts).set('Content-Type', 'application/json').send(data).end(cb);
	};

	var Tracker = (function () {
	  function Tracker(opts, global) {
	    ...
	    this.opts = _jQuery2['default'].extend(defaults, opts);
	    this.surveyFilters = [];
	    this.parameters = {};
	    ...
	    this.isReturningUser = (0, _utils.isReturningUser)();
	    this.timesVisited = (0, _utils.timesVisited)();
	    this.connectToApi();
	  }
	  ...
	  _createClass(Tracker, [{
	    ...
	    value: function connectToApi() {
	      ...
	      json(this.uiidCheckUrl, 'get', function (err, response) {
	        if (err) {}
	        var data = response.body;
	        //UUID exists
	        if (data.exists) {
	          //this.opts.surveyId = data.surveyId;
	          _this.opts.countryCode = data.countryCode;
	          _this.opts.userId = data.userId;
	          _this.opts.domainId = data.domainId;
	          _this.parameters = _jQuery2['default'].extend(_this.parameters, data.parameters);
	          _this.surveyFilters = _this.surveyFilters.concat(data.surveyFilters);
	          //make the urls
	          //this.idSelector = `survey-${this.opts.surveyId}-${this.opts.domainId}-id`;
	          _this.sendResponseUrl = _this.apiUrl + '/tracker';
	          //this.clientIP = data.clientIP ;
	          json(_this.getSurveyUrl, 'post', {
	            source: _this.trafficSource,
	            device: _this.deviceType,
	            pageTarget: _this.window.location.pathname + _this.window.location.search,
	            uuid: _this.opts.uuid,
	            domainId: data.domainId,
	            userId: data.userId,
	            countryCode: _this.opts.countryCode
	          }, function (err, response) {
	            if (err) {}
	            if (response.body) {
	              console.log(response.body);
	              //obtain view and response count from survey
	              _this.opts.cursorTrigger = response.body.cursorTrigger || false;
	              _this.opts.maxAnswers = response.body.maxAnswers || 0;
	              _this.opts.maxViews = response.body.maxViews || 0;
	              _this.viewCount = response.body.viewCount || 0;
	              _this.TTO = response.body.inPageTime;
	              _this.numberOfPages = response.body.numberOfPages;
	              _this.responseCount = response.body.responseCount || 0;
	              _this.opts.surveyId = response.body.id;
	              _this.questLength = response.body.questions.length;
	              _this.filters = response.body.filters;
	              _this.templateUrl = _this.apiUrl + '/' + _this.opts.userId + '/' + response.body.domainId + '/' + response.body.id + '/template';
	              _this.surveyPosition = response.body.position ? response.body.position : 'right';
	              _this.skipFilters = response.body.skipFilters || false;
	              //track survey calls (intents to render the survey)
	              json(_this.apiUrl + '/Surveys/' + _this.opts.surveyId + '/updateVisitCount', 'post', function (err, response) {});
	    ...
	    value: function sendResponse() {
	      ...
	      json(this.sendResponseUrl, 'post', payload, function (err, response) {
	    ...
	    value: function processFilters() {
	      ...
	      var conditionsToTest = [];
	      //lets act on some filters
	      this.filters.forEach(function (filter) {
	        //OR level grouping
	        var conditionGroupAND = true;
	        filter.group.rules.forEach(function (filterRule) {
	          //AND level grouping
	          if (!!filterRule.condition) {
	            if (filterRule.filterType == "LANG") {
	              conditionGroupAND = conditionGroupAND && filterRule.condition.toLowerCase() === _this3.language.toLowerCase();
	            }
	            if (filterRule.filterType == "VIS") {
	              if (filterRule.condition === "New" && !_this3.isReturningUser) {
	                conditionGroupAND = true;
	              } else if (filterRule.condition === "Returning" && _this3.isReturningUser) {
	                conditionGroupAND = true;
	              } else {
	            ...
	            if (filterRule.filterType == "COOKIE") {
	              var cookie = (0, _utils.getCookie)(filterRule.condition.cookieName);
	      ...
	      var resolved = conditionsToTest.reduce(function (acc, testA) {
	        return acc || testA;
	      }, false); //use OR to test
	      if (!resolved) {
	        return false;
	      }
	    ...
	    value: function showSurvey() {
	      ...
	      setTimeout(function () {
	        json(_this4.templateUrl, 'get', function (err, response) {
	          if (err) {}

	          if (_this4.opts.cursorTrigger) {
	            // is mouse is out option is selected then only trigger survey when is out of window
	            (0, _jQuery2['default'])(document).mouseleave(function () {
	              _this4.surveyBuilder(response, _this4.surveyPosition);
	            });
	          } else {
	            // regular flow is out mouse option is not selected.
	            _this4.surveyBuilder(response, _this4.surveyPosition);
	          }
	        }, 'text/html');
	      }, this.TTOperiod || 10);
	    ...
	    value: function show(id) {
	      ...
	      if (this.skipFilters) {
	        if (id) {
	          //if id passed show that survey id only if belongs to user and domain
	          this.opts.surveyId = id;
	          this.templateUrl = this.apiUrl + '/' + this.opts.userId + '/' + this.opts.domainId + '/' + id + '/template';
	        }
	        setTimeout(function () {
	          json(_this5.templateUrl, 'get', function (err, response) {
	            if (err) _this5.surveyBuilder(response, _this5.surveyPosition);
	            //displaySurvey();
	          }, 'text/html');
	        }, 10);
	      }
	    ...
	    key: 'surveyBuilder',
	    value: function surveyBuilder(response, renderPosition) {
	      var _this6 = this;

	      //return () => {
	      var startTime = new Date();
	      var endTime = undefined;
	      json(this.apiUrl + '/Surveys/' + this.opts.surveyId + '/updateViewCount', 'post', function (err, response) {});
	      //set cookies value if passed
	      if (this.parameters.COOKIE) {
	        _cookiesJs2['default'].set(this.cookieName, this.parameters.COOKIE.value, { expires: 600 }); //10 min.
	      }
	      var data = response.text;
	      var $div = (0, _jQuery2['default'])('<div>');
	      $div.addClass('alexio-wrapper').addClass(renderPosition).html(data).appendTo('body');
	      this.questLength = $div.find('.step').length;
	      this.surveyShowing = true;
	      var step = $div.find('.step');
	      var sendBtn = $div.find('.send');
	      var nextBtn = $div.find('.next');
	      var replyBtn = $div.find('.reply');
	      var radios = $div.find('.form-input-radio');
	      var surveyHandle = $div.find('.alexio-survey-handle').addClass('alexio-survey-handle-open');
	      var step0 = $div.find('.step').not('.step-0').hide();
	      var thankYouMsg = $div.find('.step-final').hide();
	      ...
	      if (this.getDeviceType() != 'Desktop') {
	        (0, _jQuery2['default'])('.hide-mobile').hide();
	        (0, _jQuery2['default'])('.show-mobile').show();
	      } else {
	        (0, _jQuery2['default'])('.show-mobile').hide();
	      }
	      ...
	      nextBtn.click(function (event) {
	        ...
	        questIteration == _this6.questLength - 1 ? nextBtn.hide() && sendBtn.show() : nextBtn.show();
	      });

	      sendBtn.click(function (event) {
	        event.preventDefault();
	        endTime = new Date();
	        _this6.timeToResponse = endTime - startTime;
	        //To hide survey when send is pressed.
	        _this6.responseReady = $div.find('form').serializeArray();
	        $div.find('.step').hide();
	      ...
	      surveyHandle.click(function (event) {
	        event.preventDefault();
	        if (_this6.surveyShowing) {
	          $div.animate({ bottom: -$div.height() });
	          surveyHandle.removeClass('alexio-survey-handle-open').addClass('alexio-survey-handle-close');
	          _this6.surveyShowing = false;
	        } else {
	          $div.animate({ bottom: 0 });
	          _this6.surveyShowing = true;
	          surveyHandle.removeClass('alexio-survey-handle-close').addClass('alexio-survey-handle-open');
	        }
	        if (surveyHandle.hasClass('alexio-survey-handle-close') && !_this6.surveyMinimized) {
	          json(_this6.apiUrl + '/Surveys/' + _this6.opts.surveyId + '/updateMinimizeCount', 'post', function (err, response) {
	            _this6.surveyMinimized = true;
	          });
	        }
	      });
	      //}
	    ...
	    value: function getDeviceType() {
	      return (0, _utils.getDeviceType)(this.window.navigator.userAgent);
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.timesVisited = timesVisited;
	exports.getCookie = getCookie;

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { 'default': obj }; }

	var _mobileDetect = __webpack_require__(8);
	...
	function isReturningUser() {
	  if (_cookiesJs2['default'].get('alex.io_visited')) {
	    //count visited times
	    _cookiesJs2['default'].set('alex.io_visited', Number(_cookiesJs2['default'].get('alex.io_visited')) + 1, { expires: 60 * 60 * 24 * 7 });
	    return true;
	  } else {
	    //set cookie
	    _cookiesJs2['default'].set('alex.io_visited', 1, { expires: 60 * 60 * 24 * 7 }); // 7 days valid cookie
	    //is new visitor
	    return false;
	  }
	}
	...
	function timesVisited() {
	  return Number(_cookiesJs2['default'].get('alex.io_visited'));
	}
...
/***/ function(module, exports) {
	...
	function deviceFilter(deviceType, devices) {
	  ...
	  var matchedDevices = devices.filter(function (current) {
	    return current === deviceType;
	  });
	  return matchedDevices.length > 0;
	}

	function ipFilter(clientIP, testIP) {
	  ...
	  var matchedIPs = testIP.filter(function (current) {
	...
	function sourceFilter(clientSource, testSource) {
	  ...
	  var matchedTrafficSource = testSource.filter(function (current) {
